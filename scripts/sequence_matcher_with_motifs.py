...
def extract_flanks(seq, pos, k, up, down):
    upstream   = seq[max(0, pos - up):pos]
    downstream = seq[pos + k:pos + k + down]
    # Pad truncated flanks so all sequences have consistent length
    upstream   = upstream.rjust(up, "N")
    downstream = downstream.ljust(down, "N")
...
def pwm_from_seqs(seqs):
    ...
    # Use the most common length; discard truncated outliers
...
def main(args):
    ...
    for key, seqs in flanks.items():
        ...
        if pwm.empty:                          # guard against all-truncated
            continue
        ic = info_content(pwm)
